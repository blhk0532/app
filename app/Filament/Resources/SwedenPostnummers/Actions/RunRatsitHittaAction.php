...
class RunRatsitHittaAction extends Action
{
    public static function make(?string $name = 'run'): static
    {
        return parent::make($name)
            ...
            ->modalDescription(fn (SwedenPostnummer $record): string => "This will queue jobs/ratsit_hitta.mjs for postnummer {$record->post_nummer} and set personer_merinfo_queue to 1.")
            ...
            ->action(function (SwedenPostnummer $record): void {
                ...
                RunRatsitHittaScriptJob::dispatch($record->post_nummer)
                ...
                Notification::make()
                    ->success()
                    ->title('Script queued')
                    ->body("Queued jobs/ratsit_hitta.mjs for {$record->post_nummer}.")
                    ->send();
            });
    }
}
