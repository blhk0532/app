...
final class RingaDataCalendar extends FullCalendarWidget implements HasCalendar
{
    ...
    public function getHeading(): string|Htmlable
    {
        ...
        return new HtmlString(view('filament.app.widgets.queue-booking-calendar-header', [
            'calendars' => BookingCalendar::all()->pluck('name', 'id'),
            'selectedTechnician' => $this->selectedTechnician,
    ...
    public function createAction(): Action
    {
        return Action::make('create')
            ...
            ->schema(function () {
                if ($this->recordId && (! isset($this->record) || ! $this->record instanceof RingaData)) {
                    $this->record = RingaData::query()->find($this->recordId);
                }
    ...
    {
        return RingaDatasResource::getEloquentQuery()
    ...
    protected function getResources(): array
    {
        ...
        for ($i = 1; $i <= 10; $i++) { // Assuming up to 10 possible filters
            ...
            if (isset($filters[$key]) && $filters[$key]) {
                $calendar = BookingCalendar::find($filters[$key]);
                if ($calendar) {
                    $resources[] = [
                        'id' => $calendar->id,
                        'title' => $calendar->name,
        ...
        if (empty($resources)) {
            $defaultCalendars = BookingCalendar::limit(5)->get(); // Or some logic to get defaults
