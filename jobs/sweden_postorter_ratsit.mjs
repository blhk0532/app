...
async function scrapeRatsitPostorter(url, kommunName, lan, connection) {
    ...
    try {
        ...
        for (const postnummerRow of postnummerMap.values()) {
            try {
                await connection.execute(
                    `INSERT INTO sweden_postnummer (post_nummer, post_ort, kommun, lan, personer, ratsit_link, is_done, is_queue)
                     VALUES (?, ?, ?, ?, ?, ?, 0, 1)
                     ON DUPLICATE KEY UPDATE
                         post_ort = VALUES(post_ort),
                         kommun = VALUES(kommun),
                         lan = VALUES(lan),
                         personer = VALUES(personer),
        ...
        for (const postort of postorterMap.values()) {
            try {
                await connection.execute(
                    `INSERT INTO sweden_postorter (post_ort, kommun, lan, personer, postnummer, ratsit_link, is_done, is_queue)
                     VALUES (?, ?, ?, ?, ?, ?, 1, 0)
                     ON DUPLICATE KEY UPDATE
                         personer = VALUES(personer),
...
async function main() {
    ...
    try {
        ...
        const [kommuner] = await connection.execute(
            'SELECT id, kommun, lan, ratsit_link FROM sweden_kommuner WHERE ratsit_link IS NOT NULL AND is_done = 0 ORDER BY id',
        );
        ...
        for (const [index, row] of kommuner.entries()) {
            ...
            if (scrapeResult !== null) {
                ...
                await connection.execute(
                    'UPDATE sweden_kommuner SET is_done = 1, is_queue = 0, postorter = ?, postnummer = ? WHERE id = ?',
