...
async function savePersonsViaApi(persons) {
  ...
  for (let i = 0; i < persons.length; i += batchSize) {
    ...
    try {
      ...
      try {
        const swedenSummary = await syncSwedenPersonsFromHittaBatch(batch);
        console.log(
          `   ✅ sweden_personer: Created: ${swedenSummary.created}, Updated: ${swedenSummary.updated}, Skipped: ${swedenSummary.skipped}, Failed: ${swedenSummary.failed}`,
        );
      } catch (swedenError) {
        console.log(`   ❌ sweden_personer batch failed: ${swedenError.message}`);
      }
    } catch (err) {
      totalFailed += batch.length;
      hittaDataFailed += batch.length;
