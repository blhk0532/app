...
TEST_POSTNUMMER = ["15332", "11115", "22223", "33332", "44447"]
COOKIES_FILE = "cookies.json"
API_URL = "http://localhost:8000/api/merinfo/bulk"
...
async def main():
    ...
    while True:
        ...
        if not postnummer:
            print("Queue empty, checking test list...")
            if postnummer_index < len(TEST_POSTNUMMER):
                postnummer = TEST_POSTNUMMER[postnummer_index]
                print(f"Using test: {postnummer}")
                postnummer_index += 1
            else:
                print("All done! Restarting test list...")
                postnummer_index = 0
