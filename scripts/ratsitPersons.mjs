...
import { closeSwedenPersonPool, syncSwedenPersonFromRatsit } from './sweden_personer_sync.mjs';
...
class RatsitScraper {
  constructor(api_url, api_token) {
    ...
    // Debug: log what URLs are set
    if (!this.api_url) {
      console.error('❌ ERROR: APP_URL is not set!');
      console.error(`   process.env.APP_URL = ${process.env.APP_URL}`);
      console.error('   Make sure .env file has APP_URL set and dotenv/config is imported');
      process.exit(1);
    }
