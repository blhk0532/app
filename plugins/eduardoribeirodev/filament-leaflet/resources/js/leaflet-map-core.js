...
// Initialize L.PM immediately to prevent geoman from failing
// Set both in module scope and on the window object for maximum compatibility
if (typeof L !== 'undefined' && !L.PM) {
    L.PM = { optIn: true };
}
// Also ensure it's available globally just in case
...
export class LeafletMapCore {
    constructor(config) {
        // Ensure L.PM is initialized before any geoman usage
        // This runs during instantiation, before createMap/init is called
        if (typeof L !== 'undefined' && !L.PM) {
            L.PM = { optIn: true };
        }
     ...
     */
    createMap() {
            // Ensure L.PM is absolutely defined right before map creation
            // This prevents geoman init hooks from failing with undefined L.PM
            if (L && !L.PM) {
                L.PM = { optIn: true };
            }
        ...
        this.map = L.map(this.config.mapId, this.config.mapConfig || {})
            .setView(this.config.defaultCoord, this.config.defaultZoom);
                ...
                // Handle errors during map initialization (e.g., L.PM not being set up correctly)
     ...
     */
    setupDrawControl() {
        const mapPm = Alpine.raw(this.map?.pm);

        if (!mapPm) {
            return;
        }

        mapPm.setLang(window.filamentData?.language);

        mapPm.setGlobalOptions({
            snappable: true,
            snapDistance: 20,
            markerStyle: {
                icon: this.createIcon()
        ...
        mapPm.addControls(this.config.mapControls.drawControls);
    }
