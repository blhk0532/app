    ...
    protected function getViewData(): array
    {
        ...
        $forecastItems = collect($forecast)
            ...
            ->map(function ($item) use ($timezoneOffset) {
                $dt = Carbon::parse($item->datetime, 'UTC')->addSeconds($timezoneOffset);
        ...
        $sunrise = $current->sunrise ? Carbon::createFromTimestamp($current->sunrise + $timezoneOffset) : null;
        $sunset = $current->sunset ? Carbon::createFromTimestamp($current->sunset + $timezoneOffset) : null;
        ...
        if ($sunriseItem) {
            $allItems = $allItems->push($sunriseItem);
        }
        if ($sunsetItem) {
            $allItems = $allItems->push($sunsetItem);
        }
